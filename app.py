...
def get_last_year():
    ...
    print(f"Body {jsonify(last_year_auto_correlation)}")
    return jsonify(last_year_auto_correlation)

@app.route("/bootcamp-final-project/api/ml-value", methods=['POST'])
@cross_origin()
def get_last_registry():
    last_year_auto_correlation = request.get_json()
    print(f"last_year_auto_correlation {last_year_auto_correlation}")
